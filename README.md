...
- **Remote**: an optional Cloudflare Tunnel (managed in-app) exposes selected surfaces to cloud clients such as Claude custom connectors and ChatGPT apps, behind bearer authentication or OAuth 2.1 + PKCE. The OAuth authorization page asks for the Apple Core token and, by default, trusts that client on this Mac in the same step, so a headless server never waits on a connection dialog nobody can see. Cloud clients may identify themselves with a Client ID Metadata Document instead of registering, which keeps one identity across reconnections rather than adding a registration each time. A tunnel belongs to the Mac that set it up: a copy of the config on a second Mac stands down at launch instead of adding a second connector, and the Access pane reports when Cloudflare sees more than one Mac on the tunnel.
...
- **Per-surface control**: each surface (Calendar, Notes, Mail, …) has one enable switch. A surface you enable is reachable wherever Apple Core is currently served, and remote requests always authenticate — access is never anonymous. Managed in a four-pane settings window (Services / Access / Clients / License), with server internals and repair actions behind a Diagnostics sheet. The License pane accepts a purchase key or imports a signed license file.
...
The standard build exposes 152 tools: Notes (32), Mail (33), Filesystem (19), Contacts (16), Reminders (14), Calendar (6), Messages (6), Capture (6), Utilities (7), Maps (6), Shortcuts (4), and Location (3). Surfaces start off except Maps and Utilities, so a fresh install advertises 13 tools until you enable more during onboarding or in Settings. The nineteen Filesystem tools additionally stay hidden until you share a folder, and six WeatherKit tools are excluded from every current build because their `WEATHERKIT_AVAILABLE` compilation condition is never set.
