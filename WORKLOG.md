...
**Not done**: home-server has not yet been updated, so its config has no owner recorded until the next release launches there; until then the protection is the MacBook's `enabled: false` plus the ownership guard in any build launched on the MacBook. The other Mac's running connector is not stopped by a take-over; that remains a manual `launchctl bootout` on that Mac.
