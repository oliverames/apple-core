...
Wrap-up corrected the README's stale GitHub binary-download link and three-pane
settings description. Live Gumroad product data, the source-only GitHub release,
and `SettingsPane` confirmed the replacements. Existing release artifacts and
...
**Immediate fix (MacBook only)**: `launchctl bootout` + `disable` of the label, plist to the Trash, and `cloudflare.enabled` set to `false` in the MacBook's local config (backup at `~/.config/apple-core/config.json.bak-2026-09-03`). Connector count returned to 1 and six consecutive `POST /mcp` probes returned 401.

**Durable fix (app)**: a tunnel now belongs to one Mac.

- `App/Services/Serving/CloudflareTunnelOwnership.swift:1` (new, dependency-free so the test target compiles it) — `CloudflareTunnelOwnership` (`unowned` / `thisMac` / `otherMac(name:)`), `CloudflareTunnelOwnershipRules.ownership(...)`, `parseTunnelInfo` for `cloudflared tunnel info --output json`, the two user-facing messages, and `MachineIdentity` (IOKit `IOPlatformUUID`, host name fallback).
- `App/Services/Serving/CloudflareManager.swift:20` — `CloudflareSettings.ownerMachineID` / `ownerMachineName` (tolerant decode, so old configs read as `unowned`); new state `CloudflareTunnelState.ownedElsewhere`; `CloudflareTunnelStatus.ownerMachineName` and `connectorCount`. `reconcileTunnel()` (startup) now stands down when another Mac owns the tunnel: it boots out and removes any local connector for the label and reports `ownedElsewhere` without changing the config. An `unowned` legacy config is adopted by the Mac that launches with it enabled, which is how home-server will claim it on its next launch. `bootstrapTunnel()` (explicit setup) refuses the same way and adopts on first setup; `takeOverTunnel()` is the deliberate override; `disableTunnel()` releases ownership. `status()` also runs `cloudflared tunnel info` while the tunnel is running here and reports `.error` with "N Macs are connected to tunnel …" when Cloudflare sees more than one connector, so the settings pane now shows the duplicate instead of a green check.
- `App/Views/RemoteAccessSetup.swift:24` — new `RemoteAccessOwnedElsewhere` block with **Take Over on This Mac** and **Turn Off Here**; `App/Views/ServingSettingsModel.swift` — `takeOverCloudflareTunnel()`.
- `Tests/CloudflareTunnelOwnershipTests.swift:1` — ownership decision, message content, connector parsing (two connectors, zero connectors, non-JSON error), machine identity stability. `Apple Core.xcodeproj/project.pbxproj` adds the new source to the test target; the App target picks it up through its synchronized folder.

**Verification**: full unit suite green via `xcodebuild … test`; `swift format lint --strict` clean on the changed files. Live: the Debug build launched four times with `APPLECORE_CONFIG_HOME` pointing at a scratch config that names a different owner; each launch logged "Cloudflare startup reconciliation: Home Server runs remote access for applecore.amesvt.com …", created no LaunchAgent plist, started no cloudflared, and left the scratch config untouched while still serving locally on its own port (401 on `/mcp`).

**Gotcha recorded**: in zsh, `log` is a shell builtin, so every `log show` / `log stream` run through the Bash tool silently returned nothing; use `/usr/bin/log`. That false negative hid the reconciliation lines on both Macs for most of this session.

**Second change, same day: trust granted from the authorization page.** The other half of what made this hard to diagnose is that Apple Core holds a new client's first connection until someone approves a dialog on the server's screen, and a headless Mac has nobody to do that. The person filling in the OAuth authorization page has already typed the master token, which is the same authority the dialog's "Always trust" asks for, so the page now carries a checkbox (on by default) that records that trust for the OAuth client ID before the code is issued. The copy says where the dialog would otherwise appear, and when `GUISession.isActive` is false it says that the screen has nobody at it and to leave the box on.

- `App/Services/Serving/AppleCoreHTTPServer.swift` — `TrustGrantHandler` and `setTrustGrantHandler`; `oauthApproveAuthorization` calls it for `.oauth(clientID:registeredName:)` when `apple_core_trust=on` after the token check; `authorizationFormHTML` adds the `.check` row and the two-state detail sentence built from `MachineIdentity.currentName()` and `GUISession.isActive`.
- `App/Controllers/ServerController.swift` — `ServerNetworkManager.setTrustGrantHandler` stores the handler and hands it to the HTTP server on start; `ServerController` sets it next to the approval handler and routes it to `addTrustedClient` on the main actor, so the grant lands in the same `trustedOAuthClientsV2` list the Settings pane shows and can withdraw.
- Verified live on the Debug build with a scratch config: `POST /oauth/register`, `GET /oauth/authorize` with the public resource renders the checkbox and the "waits for approval in a dialog on MacBook Pro" sentence, `POST /oauth/authorize` with the token and `apple_core_trust=on` returns 303 with a code, and both "trusted OAuth client from the authorization page" log lines fire. A request naming a non-canonical `resource` is still refused with 400 before any of this runs.

**Release 1.7.0 (build 24), same day.** Tag `v1.7.0` at `bf538d7`; release commit chain `f6c7655` (notes + `ENCLOSURE_URL` override in `Scripts/release.sh`), `9c6fd41` (bump), `bf538d7` (appcast). Preflight: tree clean and pushed, Sparkle `generate_keys -p` and `SUPublicEDKey` both `NIhfyD08…`, license signer `pubkey` equals the embedded `qBn20Q2x…`, Developer ID identity present, `notarytool-profile` answers, gh authenticated. `release.sh all` produced a notarized ("Accepted"), stapled build that passes `codesign`, `spctl` ("Notarized Developer ID") and `stapler validate`; zip sha256 `0e0ed213…`.

Distribution changed with this release. GitHub releases are source-only since 2026-09-02, but the appcast script still wrote a GitHub asset URL, so every appcast item would have pointed at a 404. `Scripts/release.sh` now honours `ENCLOSURE_URL`; the zip and its `.sha256` live in the `ames-website-assets` R2 bucket under `apple-core/`, served at `https://assets.amesvt.com/apple-core/Apple.Core-1.7.0.zip`, uploaded with `rclone` using the scoped R2 key from `~/.claude/.env` (`--s3-no-check-bucket` is required: the key cannot create buckets and rclone tries). Two traps recorded in `RELEASING.md`: a request made before the upload landed left a cached 404 at the edge for an hour (`cf-cache-status: HIT`, `age: 62`), and `HEAD` returned 200 while `GET` still served that 404, so only a `GET` with a checksum compare is proof. Purged both URLs through the zone API; afterwards the public download matched the local sha256 and `sign_update`'s enclosure signature over the downloaded bytes equalled the appcast item's. The appcast on `gh-pages` was fetched back from `oliverames.github.io` and is byte-identical to the committed file. The zip is also attached to the Gumroad product `H5iMAgmqjSc9_p61iSwApA==` (still a draft).

home-server: 1.7.0 carries the license gate, and home-server had no license, so before installing I signed an owner license (`license_id` `APPLE-CORE-OWNER-2026-09-03`, plan `owner`, no expiry) with the Keychain key, verified it with the signer, and placed it at `~/.config/apple-core/license.txt` with `0600`. Then a direct install per the update-cadence memory: sha256 checked on the server, `codesign --verify --deep --strict` and `spctl` on the extracted bundle before touching `/Applications`, old bundle to the Trash, `ditto` in, relaunch. Result: version 1.7.0 running, `/license-status` returns `activated: true, plan: owner`, the log shows "adopting tunnel apple-core on Home Server" and the config now carries `ownerMachineID`, one tunnel connector, and the public endpoint answers 401 three times in a row. Scratch copies of the license and zip were deleted on both Macs.

**Release 1.7.1 (build 25) and the Gumroad launch, same day.** Oliver chose Gumroad license keys over signed files for buyers ("a spot for users to paste a license key, all through Gumroad like ping-warden"). `App/Services/Serving/GumroadLicense.swift` (new, dependency-free, in the test target) carries the policy ported from Ping Warden's `LicensePolicy`: key shape, the verify request body (`increment_uses_count=false`), the verdict mapping (any `success:false`, refund, chargeback, dispute, or lapsed subscription is `revoked`; a non-JSON body is nil, never a verdict), and `GumroadLicenseRecord` with a 14-day offline grace and a 24-hour re-check interval. `LicenseGate` gained `activateGumroadKey`, `activate(input:)` (routes by shape), `reverifyGumroadKey` (background, one in flight), and a second file `gumroad-license.json` beside `license.txt`; `activationState` explains a revoked key and an expired grace separately. The Settings pane copy now names the key path and the daily check. Eight policy tests; full suite green; the live endpoint answered the sample key with 404 `success:false` exactly as the policy expects. Tag `v1.7.1` at `06faf99`; zip sha256 `a2e7e213…`; uploaded to R2 before any fetch this time, checksum and enclosure signature verified, appcast on gh-pages byte-identical, release workflow green, home-server installed directly and still activated on its owner envelope. Gumroad product `H5iMAgmqjSc9_p61iSwApA==` (permalink `applecore`): price set to $15, the license-key block inserted on the Content page (per-sale key generation), description rewritten to drop "offline, nothing phoned home" and the license-file promise, custom receipt text added, 1.7.1 zip attached, and published. No test purchase was made, on Oliver's instruction. Trap recorded: the Content editor's ⋮ menu acts on the selected node, not the row it sits beside; the first Delete removed the license-key block, and neither the menu, the keyboard, nor the expanded panel would remove a file row. What worked: the TipTap editor is reachable from the ProseMirror DOM as `document.querySelector('.ProseMirror').pmViewDesc.children[i].spec.editor`; `deleteRange` over the 1.7.0 node (checked by its file id `wbvIb8…` first) and `insertContent({type: 'licenseKey'})` restored the intended content, and Save persisted it: the reloaded editor holds EULA, 1.7.1, and the license-key block. The CLI still lists the 1.7.0 upload in the product's file store, but it is no longer embedded, so buyers never see it. Cowork Global instructions were synced by inserting the one new sentence into the live field after proving it equalled the old snapshot by SHA-256, then proving the saved field equals the new snapshot; chat preferences were already identical.

**Not done**: The other Mac's running connector is not stopped by a take-over; that remains a manual `launchctl bootout` on that Mac. The 1.7.0 upload remains in Gumroad's file store for the product (not embedded, not visible to buyers); Gumroad offers no CLI or API delete for it.

## 2026-09-02 - Dual-licensed binary, Ed25519 license gate, and Gumroad distribution

**What changed**: What began as a licensing question — "what would Gumroad licensing look like for this project?" — became a full distribution pivot. The repo keeps the source GPL-3.0-or-later and conveys the official signed `.app` under `EULA.md` instead, with an offline Ed25519 license file that unlocks MCP serving. An unlicensed copy returns `402 Payment Required` on every MCP session-creating request and directs the buyer to **Settings → License**.

**Code — license verification and gate (soft gate by design; DIY builds from GPL source stay free):**

- `Shared/LicenseDocument.swift:1` — envelope format, `LicenseDocument` (`license_id`/`product`/`plan`/`licensed_to`/`issued_at`/`expires_at`), `LicenseDocumentCodec` (three-line `APPLE-CORE-LICENSE-1` + base64 payload + base64 Ed25519 signature over the exact payload bytes, 8192-byte cap, products check `apple-core`), expiry handling. The public half will be baked into the binary; the private half lives only in the login Keychain and 1Password.
- `App/Services/Serving/LicenseGate.swift:1` — actor that reads `AppleCoreServingPaths.licenseURL()` (`App/Services/Serving/ServingConfig.swift:149`), verifies with `AppleCoreLicensePublicKey`, caches the validated document, re-reads on file mtime change, and exposes `ActivationState` (`notActivated`/`active`/`rejected`) plus `activate`/`deactivate` (activate verifies first, only then writes `0600` to `license.txt`; deactivate moves the file to Trash with a fallback to `removeItem`). `AppleCoreLicensePublicKey.base64` is currently `REPLACE_AT_RELEASE` and must be set before the first signed license will verify. The 1Password backup mirrors the Sparkle key's discipline.
- `App/Services/Serving/AppleCoreHTTPServer.swift:42` — new `licenseGate` property (default `LicenseGate()`), a license check inserted into `POST /mcp` (`App/Services/Serving/AppleCoreHTTPServer.swift:278`), `GET/DELETE /mcp`, `GET /sse` and `POST /message` after `authenticate`. The new `GET /license-status` (`App/Services/Serving/AppleCoreHTTPServer.swift:244`) is intentionally unauthenticated so a locked-out client or a support conversation can distinguish "not activated" from "server broken". The gated routes answer `HTTP 402 Payment Required` via `licenseRequiredResponse` (`App/Services/Serving/AppleCoreHTTPServer.swift:1038` — `License required: activate Apple Core in Settings → License to serve MCP.`). `statusResponse` stays authenticated and diagnostic-only. The server's `init` now takes `licenseGate: LicenseGate? = nil` and defaults to `.shared` (`App/Services/Serving/AppleCoreHTTPServer.swift:42`).

**Code — Settings UI:**

- `App/Views/SettingsView.swift:22` — `SettingsPane` gains `.license = "License"` (`key` symbol). The new `LicensePane` (`App/Views/SettingsView.swift:678`) shows `Licensed to`/`Expires`/`License ID` when active, a paste `TextEditor` plus **Activate** when not, and **Deactivate…** when active. The pane refreshes via `ServingSettingsModel.refreshLicenseState()` on `.task`.
- `App/Views/ServingSettingsModel.swift:53` — `@Published licenseState`/`licensePasteText`/`isActivatingLicense`/`licenseActivationError` and `refreshLicenseState()`/`activateLicense()`/`deactivateLicense()` (all routing through the shared `LicenseGate` actor).
- `App/Info.plist` is unchanged; the Sparkle `SUPublicEDKey` comparison in `RELEASING.md` stays the sole Info.plist key check. The license key previously lived there in the plan but was moved to the `LicenseGate` constant to keep app and tests in sync without Info.plist gymnastics.

**Code — project plumbing:**

- `Shared/LicenseDocument.swift` added to *both* the `Apple Core` app target (auto via the synchronized `App/` group for the app-side, but `Shared/` is a plain group) and the `Apple CoreTests` bundle via two new `PBXBuildFile` entries (`AC00000100000000000F2`/`F3` with fileRef `AC00000100000000000F1`) and the `Shared` group child. This mirrors the `Shared/FilesystemAccess.swift:031/032` pattern that existing tests rely on. `App/Services/Serving/LicenseGate.swift` and the new `App/Views` pieces are covered by the synchronized `App/` root group — no explicit `pbxproj` entry needed.
- `Scripts/sign_license.swift:1` — standalone `swift` tool with `genkey` (creates a `Curve25519.Signing.PrivateKey`, stores its 32-byte `rawRepresentation` in the login Keychain under `com.oliverames.applecore.license-signing` / `license-signing-key` as a generic password, prints the public half for embedding), `pubkey`, `sign <json> [output.txt]` (canonicalizes the `LicensePayload` through the same `JSONEncoder`/`JSONDecoder` with `.iso8601`/`.sortedKeys` that the app decodes with, signs the exact bytes), and `verify <envelope>`. Never prints the private key; its docstring points at the 1Password backup step.
- `Tests/LicenseDocumentTests.swift:1` — 8 tests with Swift Testing (`@Suite("License document envelope")`): round-trip, tampered payload is rejected, wrong-key rejection, signature-replay (moved signature over different payload bytes), junk as malformed, wrong product, expired vs valid-dated, and perpetual expiry.

**Docs and plan:**

- `EULA.md:1` — binary license for the signed, notarized `.app` Oliver distributes via Gumroad. One-person, perpetual for the major version, non-exclusive, non-transferable, non-redistributable for the signed binary itself; source `LICENSE.md` (GPL-3.0-or-later) is explicitly left intact ("Nothing in this agreement restricts any right you hold under the GPL for that source code"). Warranty disclaimer and liability cap mirror the GPL disclaimer. Vermont governing law.
- `CONTRIBUTING.md:1` — inbound `GPL-3.0-or-later` plus a grant to Oliver to relicense/sublicense under the EULA (so sole copyright survives future PRs).
- `docs/licensing.md:1` — runbook covering the dual-license split, the offline Ed25519 verification (shared doc + gate + signer script), the Gumroad fee/tax picture, product creation gotchas discovered live, per-sale signing and delivery, appcast policy, and the release-gate key-match check. Gumroad's `https://gumroad.com/pricing` and help center were checked 2026-09-02; the license-key tooling was behind sign-in, so the docs describe a signer-independent flow.
- `docs/planning/BUILD_PLAN.md:4` — `§0` row 4 and the opening decision note now point at §4.4. New `§4` ("Per §0 decision 4 (amended 2026-09-02 — see §4.4)") and `§4.4 Dual-license decision (2026-09-02)` record the product/source split, the consequence that no GPL-licensed third-party code may enter the binary, and the restoration of the clean-room discipline for Mail v2.0 (SYNTHESIS.md had already priced that work at two to three weeks). The v2.0 Mail bullets were reworded from "lift directly" to "clean-room from documented `apple-mail-mcp` behavior".
- `README.md:16` — *Origin and license* now distinguishes source (GPL-3.0-or-later) from the signed binary (conveyed under `EULA.md` via Gumroad, offline file verified in **Settings → License**; "An unlicensed copy does not allow setting up any MCP").
- `NOTICE:8` — adds the binary paragraph (signed distribution under `EULA.md`, no GPL third-party code, `apple-mail-mcp` clean-room reference only) under the existing iMCP MIT paragraph.
- `RELEASING.md:19` — release gate now checks that the binary is not incorporating GPL-licensed third-party code and that the embedded public key still verifies against the Keychain private key (`swift Scripts/sign_license.swift pubkey` round-trip). Publisher notes clarify that GitHub releases no longer carry the DMG.
- `.github/workflows/release.yml:37` — `Publish GitHub release (source + notes only; no DMG — see docs/licensing.md)` with a comment that the signed zip is Gumroad-only; the `gh release create` call intentionally carries no asset.
- `Scripts/release.sh:646` — `upload_asset` now refuses: it prints the Gumroad guidance and the legacy `gh release upload` escape hatch, then `exit 1`. The help entry for `upload` stays so a caller sees the new failure rather than "Unknown command".
- `appcast.xml:70` — unchanged in this commit; historical `enclosure` entries still point at `github.com/.../releases/download`. The docs note that the next signed enclosure should be published via the Gumroad product instead.

**Distribution — Gumroad:**

Gumroad CLI (`/opt/homebrew/bin/gumroad`, version 2026.08.18, logged in as `oliverames@gmail.com` per `gumroad auth status`) was exercised against the live account. The pre-existing product `FmGG0pxyEyzJqp_BG4itFQ==` (`Ping Warden License`, unpublished) confirmed the API shape. A new draft was created:

- `gumroad products create --name "Apple Core License" --price 25` → product `H5iMAgmqjSc9_p61iSwApA==`, short URL `https://olivera40.gumroad.com/l/klcykp`, then `gumroad products update H5iMAgmqjSc9_p61iSwApA== --custom-permalink applecore` → `https://olivera40.gumroad.com/l/applecore` (still `published: false`; attach the DMG and publish when the first keyed binary is ready). `products create` fails without `--cover` even with `--unpublished`; `--price` is indexed to USD — the flag is `--custom-summary`, not `--custom_summary` (discovered via `invalid number` and the `cover is required` error during the live run). `docs/licensing.md:60` records the clean sequence that reaches a product page.

**Decisions made**: The initial `LicenseDocument.swift` put the `licenseFileURL` helper inside `AppleCoreServingPaths` alongside the three existing paths (`Apple Core.xcodeproj/project.pbxproj:0 — `Shared/` is not synchronized, so that helper would have needed duplicating. Moving the path to `LicenseGate` kept `Shared/` pure and testable and let the serving layer supply the config-home-aware URL.

Adopted a *three-line* envelope (`APPLE-CORE-LICENSE-1` / base64 payload / base64 signature). The signature covers the exact payload bytes, never a re-serialization, so no canonical JSON is required and the same bytes a buyer pastes are the bytes verified.

Kept the gate *outside* `AppleCoreServingConfig` and wired it as a post-auth, pre-dispatch check in `AppleCoreHTTPServer`. Localhost is deliberately not exempt: the EULA activation term covers setting up any MCP session, wherever the request originates (recorded in `EULA.md` and `docs/licensing.md`). `GET /license-status` was added as the one unauthenticated license-aware endpoint so a locked-out client can tell "license missing" from "server broken". Delivery of the license itself stays a simple *paste* into the Settings pane (not a file-chooser only), which tolerates the trailing-newline and `\\r\\n` variations Gumroad/custom receipts introduce.

Chose *dual licensing* with a *soft* offline gate rather than a network check: anyone may build and redistribute a binary from the GPL source without a license file, and that build is simply not the signed, notarized product. The moat is the signature, not secrecy. `apple-mail-mcp` therefore reverted to architecture-reference only — its GPL-3.0 code cannot enter the EULA binary — at the already-priced clean-room cost.

Pushed GitHub releases to be *source + notes only* per the explicit 2026-09-02 direction ("Make sure we do not include a bundled DMG in GitHub releases going forward"). The `release.sh upload` hard refusal and the workflow's `no binary asset` comment are the guardrails.

**Verification**: `xcodebuild ... -configuration Debug build` (`** BUILD SUCCEEDED **` after a `pbxproj` truncation — the first LICENSE patch had silently dropped the final `0` of the `AC000001000000000000000E` fileRef, which made `ServiceAccessPolicy.swift` resolve to the project root; the stale `DerivedData` hid the break until a clean `DerivedData`-pruned build). The `"String does not conform to Error"` in `LicenseGate.swift:117` was `Result<LicenseDocument, String>` → `Result<LicenseDocument, LicenseActivationError>` with a `LicenseActivationError(message:)` wrapper. `swift format lint --strict --recursive .` was clean; a second pass after the `SettingsView` long-line fix introduced four new `AddLines`/`Indentation` failures (multiline array literals in the test `tampered`/`frankenvelope` helpers) which were then rewritten to string-interpolated envelopes and re-linted.

`xcodebuild ... test` `** TEST SUCCEEDED **` with 8 new `LicenseDocumentTests` passing alongside the existing suite (addressed: `roundTrip`, `tamperedPayload`, `wrongKey`, `signatureReplay`, `junkInput`, `wrongProduct`, `expiry`, `noExpiry`). A pre-push `git status --porcelain` caught the second `git add`/staged-nothing trap (the staged index still held the pre-stale-cache `project.pbxproj`); `xcodebuild -list` confirmed the project parses and `swift format lint --strict` confirmed the tree before staging.

Manual `VERSION=9.9.9 Scripts/release.sh upload` was exercised live on the committed tree and correctly prints the Gumroad guidance and exits `1` (not `0`). The funnel was also exercised end to end: the project builds before that check, so the check is not masking a broken build.

**Left off at**: Commit `b834245` (`license: dual-license with Ed25519 gate for Gumroad binary`, 18 files, 1144 insertions, 41 deletions) pushed; `main` matches `origin/main`. `EULA.md`, `CONTRIBUTING.md`, `docs/licensing.md`, and the `License pane` are in the tree. The Gumroad draft product (`H5iMAgmqjSc9_p61iSwApA==`) exists unpublished at `gumroad.com/l/applecore` (`$25`, short URL `https://olivera40.gumroad.com/l/klcykp` then `applecore`); no DMG is attached yet, and no GitHub release bears a DMG from this commit forward. The new `402` path is reachable but the server returns a hard `401` today because the app has no embedded `AppleCoreLicensePublicKey` beyond the `REPLACE_AT_RELEASE` placeholder — generation and embedding (`Scripts/sign_license.swift genkey` → Keychain item `com.oliverames.applecore.license-signing` / `license-signing-key` → `1Password` Secure Note → paste `base64(publicKey)` into `App/Services/Serving/LicenseGate.swift:13`) is still the operator step before the first keyed binary. A personal `~/.config/apple-core/license.txt` for dev can be written now or deferred; the tree tolerates either because the placeholder key makes every license file `invalidSignature` until the real one ships.

**Open questions**:

- Generate and embed the production Ed25519 keypair before tagging the next binary, and create the `Apple Core License EdDSA Private Key` 1Password note (the `op` CLI, not a bare shell pipe under `${VAR:-}`) that documents which public key it matches — the same discipline as the Sparkle key's "Private key for signing Sparkle updates" note. Only then `sign_license` round-trips and the Settings **Activate** button verifies the real `AppleCoreLicensePublicKey.base64`.
- Decide whether an unlicensed `Download` flow should freeze `appcast.xml` at the last free build or publish a second, Gumroad-hosted enclosure; the current `RELEASING.md` notes both possibilities without picking one.
- Publish the Gumroad product (attach the notarized `.zip`, set the cover image to satisfy `--cover`, confirm `<$25` vs tiered pricing) and wire its fulfillment (email-delivered env file or Gumroad license-key string) so a buyer receives a `APPLE-CORE-LICENSE-1` file with their purchase and can re-download it.
...
The release surfaced a real defect in `release.sh`. Last session's fix for interrupted appcast runs wrote the working copy to `appcast.xml.next`, and `sign_update` decides whether to embed a feed signature or merely print an enclosure signature from the file extension. A name ending in `.next` is not an appcast to it, so nothing was embedded, `--verify` failed demanding an explicit signature argument, and the copy still carried 1.0.8's trailing signature over different bytes. That is precisely the stale-signed feed `SURequireSignedFeed` clients reject, which is the failure the sibling-file change existed to prevent. Renamed the sibling to `appcast.next.xml`, which keeps the interruption safety and restores detection, and documented the extension requirement at the code so the trap is not re-set.

**Decisions made**: Numbered 1.1.0 rather than 1.0.9, because the span adds user-facing surfaces and reshapes settings rather than only fixing things. The failed appcast step was re-run against the already-notarized artifacts instead of rebuilding, since the zip that Sparkle signs is the one notarization already accepted.

**Verification**: Preflight before cutting: strict lint clean, gitleaks clean over 219 commits, `TEST SUCCEEDED` with no failures. Sparkle keypair checked before signing per RELEASING.md, `generate_keys -p` and `SUPublicEDKey` both `NIhfyD08...`. `release.sh all` produced a Developer ID signed, notarized, stapled build that passes `codesign`, `spctl` ("accepted", "Notarized Developer ID") and `stapler validate`. The release workflow's CI gate passed on `macos-26` before publishing. End-to-end after publishing, not just locally: the asset downloaded from the release hashes identically to the signed zip (`d0dadb25...`), the appcast fetched from `oliverames.github.io` matches the committed file byte for byte, its feed signature verifies against the shipped public key, and a fresh enclosure signature computed over the downloaded asset equals the one the feed advertises.

**Left off at**: 1.1.0 published with its asset, appcast live on `gh-pages`, working tree clean.
