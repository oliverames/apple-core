...
- **MCP compliance updated** (`docs/planning/MCP_COMPLIANCE.md`): protocol 2025-06-18 negotiates successfully; the installed 1.0.0 build exposes 77 unique tools locally; all 77 now declare `outputSchema` and return `structuredContent` on success. Codex 0.144.6 accepts the live bearer configuration, and a direct authenticated handshake enumerates all 77. A model-driven Codex run remains separately gated because it would disclose the live tool inventory to an external model session.
...
- **Release-readiness pass substantially complete**: a real Swift Testing target now runs in CI; the opt-in runtime harness verifies 77-tool enumeration plus reversible writes and gates Apple-account CRUD behind named disposable containers; remote surfaces fail closed; Cloudflare and Apple Core run as repaired LaunchAgents from stable paths; the live public endpoint passes authenticated default-deny checks; donor license copies are current; and stale LaunchServices registrations were removed. Reminders and Contacts TCC calls pass, while Calendar still needs its user-controlled Privacy & Security toggle. The existing `notarytool-profile` was verified against Apple's service; the remaining signing gate is importing the Developer ID Application certificate already stored in 1Password. Disposable-account names and explicit approval for a model-driven Codex inventory run also remain external gates. Oliver confirmed the gray Swift-rendered SF Symbols icon is the intended artwork. The first release cut still explicitly awaits Oliver's go.
