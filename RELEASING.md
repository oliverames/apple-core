...
- Regenerate `App/Resources/ThirdPartyNotices.txt` with `python3 Scripts/generate_third_party_notices.py <SourcePackages/checkouts>`, supplying additional checkout directories if needed. The generator reads license and notice files from the exact revisions in `Package.resolved`. Run it with `--check` before release, and verify the exported app contains the identical `Contents/Resources/ThirdPartyNotices.txt`. Repeat after dependency changes and downstream merges. This packaging check does not replace the redistribution-rights audit.
